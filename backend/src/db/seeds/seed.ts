...
function randomHourInDay(baseDate: Date, hour: number): Date {
...
export async function runSeed(): Promise<void> {
  const existsResult = await pool.query('SELECT COUNT(*) as count FROM gyms');
  ...
  // We'll seed members in gym batches. Track each member for later checkin seeding.
  interface MemberRecord {
    ...
    churn_risk?: 'HIGH' | 'CRITICAL' | null;
  }

  const allMembers: MemberRecord[] = [];

  const memberRows: [string, string, string, string, string, string, string, string, string, string][] = [];
  ...
  for (let gi = 0; gi < GYMS.length; gi++) {
    ...
    for (let mi = 0; mi < count_for_gym; mi++) {
      ...
      let plan_type: 'monthly' | 'quarterly' | 'annual';
      if (planRoll < cfg.monthly) plan_type = 'monthly';
      else if (planRoll < cfg.monthly + cfg.quarterly) plan_type = 'quarterly';
      else plan_type = 'annual';

      // Member type: 80% new, 20% renewal
      const member_type: 'new' | 'renewal' = Math.random() < 0.80 ? 'new' : 'renewal';

      // Status
      const statusRoll = Math.random();
      let status: 'active' | 'inactive' | 'frozen';
      if (statusRoll < cfg.activePct) status = 'active';
      else if (statusRoll < cfg.activePct + 0.08) status = 'inactive';
      else status = 'frozen';

      // joined_at
      let joined_at: Date;
      if (status === 'active') joined_at = daysAgo(rand(1, 90));
      else joined_at = daysAgo(rand(91, 180)); // inactive members joined longer ago
      ...
      // Assign churn risk
      let churnRisk: 'HIGH' | 'CRITICAL' | null = null;
      if (status === 'active') {
  ...
  const membersByGym: MemberRecord[][] = Array.from({ length: GYMS.length }, () => []);
  ...
  let ciBuffer: { member_id: string; gym_id: string; checked_in: Date; checked_out: Date }[] = [];

  async function flushCheckins() {
    if (ciBuffer.length === 0) return;
    const values: unknown[] = [];
    ...
    await pool.query(
      `INSERT INTO checkins (member_id, gym_id, checked_in, checked_out) VALUES ${placeholders.join(',')}`,
      values
    );
    ciBuffer = [];
  }
  ...
  for (const cfg of openCheckinConfig) {
    ...
    if (openPlaceholders.length > 0) {
      await pool.query(
        `INSERT INTO checkins (member_id, gym_id, checked_in) VALUES ${openPlaceholders.join(',')}`,
        openValues
      );
    }
  }
  ...
  for (const m of highRiskMembers) {
    ...
    await pool.query(
      `UPDATE members SET last_checkin_at = NOW() - ($1 || ' days')::INTERVAL WHERE id = $2`,
      [daysBack, m.id]
    );
  }
  ...
  const PLAN_PRICE: Record<string, number> = { monthly: 1499, quarterly: 3999, annual: 11999 };
  const payBuffer: { member_id: string; gym_id: string; amount: number; plan_type: string; payment_type: 'new' | 'renewal'; paid_at: Date }[] = [];

  async function flushPayments() {
    if (payBuffer.length === 0) return;
    const values: unknown[] = [];
  ...
  for (let i = 0; i < 10; i++) {
    ...
    await pool.query(
      ...
      [m.id, saltLakeId, 1999, 'quarterly', paid_at.toISOString()]
    );
  }
  ...
  await pool.query(
    `INSERT INTO payments (member_id, gym_id, amount, plan_type, payment_type, paid_at)
     VALUES ($1, $2, $3, $4, 'new', $5)`,
    [saltMember.id, saltLakeId, 1499, 'monthly', todayMorning.toISOString()]
  );
