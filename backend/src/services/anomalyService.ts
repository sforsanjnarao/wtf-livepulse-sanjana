...
import type { AnomalyDetectedEvent, AnomalyResolvedEvent } from '../types/index';

// ─── Detect: zero check-ins ────────────────────────────────────────────────
export async function detectZeroCheckins(): Promise<void> {
  const now = new Date();
  ...
  for (const gym of r.rows) {
    ...
    const existing = await pool.query(
      `SELECT id FROM anomalies WHERE gym_id = $1 AND type = 'zero_checkins' AND resolved = FALSE LIMIT 1`,
      [gym.id]
    );
    ...
    const ins = await pool.query(
      `INSERT INTO anomalies (gym_id, type, severity, message)
       VALUES ($1, 'zero_checkins', 'warning', $2) RETURNING *`,
      [gym.id, msg]
    );
...
export async function detectCapacityBreach(): Promise<void> {
  const r = await pool.query(`
  ...
  for (const gym of r.rows) {
    const existing = await pool.query(
      `SELECT id FROM anomalies WHERE gym_id = $1 AND type = 'capacity_breach' AND resolved = FALSE LIMIT 1`,
      [gym.id]
    );
    if (existing.rows.length > 0) continue;
    ...
    const ins = await pool.query(
      `INSERT INTO anomalies (gym_id, type, severity, message)
       VALUES ($1, 'capacity_breach', 'critical', $2) RETURNING *`,
      [gym.id, msg]
    );
    const anomaly = ins.rows[0];
...
// ─── Detect: revenue drop ──────────────────────────────────────────────────
export async function detectRevenueDrop(): Promise<void> {
  const r = await pool.query(`
    WITH today_rev AS (
    ...
    SELECT lw.gym_id AS id, g.name, lw.revenue AS last_week, COALESCE(tr.revenue, 0) AS today
  ...
  for (const gym of r.rows) {
    const existing = await pool.query(
      `SELECT id FROM anomalies WHERE gym_id = $1 AND type = 'revenue_drop' AND resolved = FALSE LIMIT 1`,
      [gym.id]
    );
    if (existing.rows.length > 0) continue;
    ...
    const ins = await pool.query(
      `INSERT INTO anomalies (gym_id, type, severity, message)
       VALUES ($1, 'revenue_drop', 'warning', $2) RETURNING *`,
      [gym.id, msg]
    );
    const anomaly = ins.rows[0];
...
export async function resolveAnomalies(): Promise<void> {
  // Resolve zero_checkins when check-ins resume
  const zeroCheckinAnomalies = await pool.query(
    `SELECT id, gym_id FROM anomalies WHERE type = 'zero_checkins' AND resolved = FALSE`
  );
  for (const a of zeroCheckinAnomalies.rows) {
    const recent = await pool.query(
      `SELECT id FROM checkins WHERE gym_id = $1 AND checked_in >= NOW() - INTERVAL '2 hours' LIMIT 1`,
      [a.gym_id]
    );
  ...
  const capBreaches = await pool.query(
    `SELECT a.id, a.gym_id, g.capacity FROM anomalies a
  ...
  const revDrops = await pool.query(
    `SELECT id, gym_id FROM anomalies WHERE type = 'revenue_drop' AND resolved = FALSE`
  );
...
async function resolveAnomaly(anomalyId: string, gymId: string): Promise<void> {
  await pool.query(
    `UPDATE anomalies SET resolved = TRUE, resolved_at = NOW() WHERE id = $1`,
    [anomalyId]
  );
  const evt: AnomalyResolvedEvent = {
